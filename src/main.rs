...
pub struct LevelColors {
    pub debug_color: Option<PatternColor>,
    pub info_color: Option<PatternColor>,
    pub warning_color: Option<PatternColor>,
    pub error_color: Option<PatternColor>,
}

impl LevelColors {
    pub fn with_debug_color(self, debug_color: Option<PatternColor>) -> Self {
        Self { debug_color, ..self }
    }

    pub fn with_info_color(self, info_color: Option<PatternColor>) -> Self {
        Self { info_color, ..self }
    }

    pub fn with_warning_color(self, warning_color: Option<PatternColor>) -> Self {
        Self { warning_color, ..self }
    }

    pub fn with_error_color(self, error_color: Option<PatternColor>) -> Self {
        Self { error_color, ..self }
    }

    pub fn match_color<T: AsRef<str>>(&self, line: T) -> Option<Color> {
        self.debug_color
            .as_ref()
            .filter(|pattern| pattern.regex.is_match(line.as_ref()))
            .or_else(|| {
                self.info_color
                    .as_ref()
                    .filter(|pattern| pattern.regex.is_match(line.as_ref()))
...
impl<B: BufRead> ColorizeLines<B> {
    pub fn new(level_colors: LevelColors, lines: Lines<B>) -> Self {
        Self {
            level_colors,
            lines,
            current_color: None,
...
impl<B: BufRead> Iterator for ColorizeLines<B> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        self.lines.next().and_then(|line_result| line_result.ok()).map(|line| {
            if let Some(color) = self.level_colors.match_color(&line) {
...
#[cfg(test)]
mod tests {
    use super::{ColorizeLines, Events, LevelColors, PatternColor, States};
    use ansi_term::Color;
    use regex::Regex;
    use std::{
        fs::File,
